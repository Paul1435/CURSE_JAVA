...
public record Reader() {
    ...
    public static Lot readLot(String path, long second) throws IOException {
        ...
        try {
            beginPrice = new BigDecimal(data[1]);
        } catch (RuntimeException e) {
            throw new RuntimeException("В файле отсутствует стартовая цена");
        }
        if (beginPrice.compareTo(BigDecimal.ZERO) < 0) {
            throw new RuntimeException("Неверно задана цена");
        }
        return new Lot(data[0], LocalDateTime.now().plusSeconds(second), beginPrice);
    }
