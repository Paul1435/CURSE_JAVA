...
public class DataBaseTableHelper {
    ...
    private DataBaseTableHelper(DataBaseBuilder builder) {
    ...
    public String toString() {
        ...
        try {
            checkConnection();
            String[] columns = fieldsOfTable.split(",");
            String firstColumn = columns[0].substring(columns[0].indexOf("(") + 1).trim();
            String getNameFirstColumn = firstColumn.split(" ")[0].trim();
            result = new StringBuilder(tableName + "\n");
            try (Statement statement = connection.createStatement()) {
                String selectQuery = "SELECT * FROM " + tableName + " ORDER BY " + getNameFirstColumn + " ASC";
    ...
    static public class DataBaseBuilder {
        ...
        public DataBaseTableHelper build() {
            return new DataBaseTableHelper(this);
        }
    }
